# -*- coding: utf-8 -*-
"""
Created on Mon Mar 30 23:57:12 2026
...
merged_excel_path = os.path.join(output_folder, "Merged_BDFI_PM25_Data_UPDATED.xlsx")
daily_excel_path = os.path.join(output_folder, "Daily_Averaged_BDFI_PM25_UPDATED.xlsx")
boxplot_path = os.path.join(output_folder, "Figure_7_2_1_IO_Boxplot_UPDATED.png")
daily_plot_path = os.path.join(output_folder, "Daily_Averaged_Indoor_Outdoor_PM25_UPDATED.png")
...
print("Merged rows before removing bad periods:", len(df))
print("Date range before filtering:", df["Time"].min(), "to", df["Time"].max())

# =====================================================
# 6. REMOVE KNOWN BAD / UNAVAILABLE DATA PERIODS
# =====================================================
...
print("Rows after I/O filtering:", len(df_io))

# =====================================================
# 8. CREATE I/O BOXPLOT
# =====================================================
...
ax.set_title("Distribution of Indoor–Outdoor PM₂.₅ Ratios in the BDFI Building", fontsize=13)
...
# =====================================================
# 9. PRINT I/O SUMMARY STATISTICS
# =====================================================

summary = pd.DataFrame({
    "Metric": ["Median", "Mean", "25th percentile", "75th percentile"],
    "First Floor": [
        df_io["FF_IO"].median(),
        df_io["FF_IO"].mean(),
        df_io["FF_IO"].quantile(0.25),
        df_io["FF_IO"].quantile(0.75),
    ],
    "Ground Floor": [
        df_io["GF_IO"].median(),
        df_io["GF_IO"].mean(),
        df_io["GF_IO"].quantile(0.25),
        df_io["GF_IO"].quantile(0.75),
    ]
})
...
# =====================================================
# 10. DAILY AVERAGED INDOOR AND OUTDOOR PM2.5
# =====================================================
...
# ensure missing days remain as gaps
daily_avg = daily_avg.asfreq("D")

# save daily averages
daily_avg.to_excel(daily_excel_path)
print("\nDaily averaged data saved to:")
print(daily_excel_path)

# =====================================================
# 11. PLOT DAILY AVERAGED PM2.5
# =====================================================
...
print("Daily averaged plot saved to:")
